...
class Board:
	...
	def update(self):
		...
		for x in range(8):
			for y in range(8):
				if (human_vs_AI and self.player == 1) or (AI_vs_human and self.player == 0) or random_vs_AI or AI_vs_random:
	...
	def aiMove(self):
		if not self.won:
			...
			if ((human_vs_AI or random_vs_AI) and self.player == 0) or ((AI_vs_human or AI_vs_random) and self.player == 1):
				startTime = time()
				self.oldarray = self.array
				alphaBetaResult = self.alphaBeta(self.array,depth,-float("inf"),float("inf"),1)
				self.array = alphaBetaResult[1]
				print(alphaBetaResult[0])
				print('===')
				if len(alphaBetaResult)==3:
					position = alphaBetaResult[2]
					if AI_vs_human or AI_vs_random:
						# print('B')
...
def dumbScore(array,player):
	...
	if human_vs_AI or random_vs_AI:
		player = 0
	else:
		player = 1
...
def slightlyLessDumbScore(array,maximizing):
	score = 0
	mobility = 0

	curPlayer = 1-maximizing
	...
	for x in range(8):
		for y in range(8):
			...
			##Check mobility
			if valid(array,1-curPlayer,x,y):
...
def decentHeuristic(array,maximizing):
	score = 0
	cornerVal = 120
	adjacentVal = 20
	sideVal = 20
	...
	for x in range(8):
		for y in range(8):
			...
			#Check swipeOut
			if (array[x][y] == 'b' and curPlayer == 0) or (array[x][y] == 'w' and curPlayer == 1):
				swipeOut = 0
...
def clickHandle(event):
	global depth
	global random_vs_AI,AI_vs_random,human_vs_AI,AI_vs_human
	xMouse = event.x
	yMouse = event.y
	if running:
...
def runGame():
    ...
    #Creating the agent buttons
	for i in range(4):
		#Background
		screen.create_rectangle(15+125*i, 410, 115+125*i, 455, fill="#000", outline="#000")
		screen.create_rectangle(15+125*i, 400, 115+125*i, 450, fill="#111", outline="#111")

		screen.create_text(65,425,anchor="c",text="Rnd vs AI",font=("Consolas", 15),fill="red")
		screen.create_text(190,425,anchor="c",text="AI vs Rnd",font=("Consolas", 15),fill="red")
		screen.create_text(315,425,anchor="c",text="P vs AI",font=("Consolas", 15),fill="red")
		screen.create_text(440,425,anchor="c",text="AI vs P",font=("Consolas", 15),fill="red")
	
	#Creating starting button
	screen.create_rectangle(180, 500, 310, 550, fill="#111", outline="#111")
	screen.create_rectangle(180, 505, 310, 555, fill="#000", outline="#000")

	screen.create_text(245,530,anchor="c",text="START",font=("Consolas", 15),fill="green")
